 ...
 */
    ...
    @AnyThread
    @VisibleForTesting
    internal fun recordResult(@NonNull wrapper: RequestWrapper, throwable: Throwable?) {
        var report: StatusReport? = null
    ...
    fun retryAllFailed(): Boolean {
        val toBeRetried = arrayOfNulls<RequestWrapper>(
            RequestType.values().size)
        var retried = false
        synchronized(mLock) {
    ...
    internal inner class RequestQueue(
